...
const createProgram = ({gl, vertex, fragment, attributes = [], uniforms = {}}) => {
  ...
  const locations = Object.keys(uniforms).reduce((newLocations, name) => {
    return {...newLocations, [name]: gl.getUniformLocation(programId, name)};
  }, {});
...
const loadUniform = function({gl, type, name, locations, value}) {
  switch (type) {
      case 'i':
          gl.uniform1i(locations[name], value);
          break;
      case 'f':
          gl.uniform1f(locations[name], value);
          break;
      case 'fv':
          gl.uniform1fv(locations[name], new Float32Array(value));
          break;
      case 'vec3':
          gl.uniform3fv(locations[name], new Float32Array(value));
          break;
      case 'vec2':
          gl.uniform2fv(locations[name], new Float32Array(value));
          break;
      case 'mat4':
          gl.uniformMatrix4fv(locations[name], false, new Float32Array(value));
          break;
      default:
  }
};

const createModel = (gl, attributes, coords) => {
  return Object.keys(attributes).reduce((model, attribute) => {
      let buffer = gl.createBuffer();
      if (attribute === 'index') {
          gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, buffer);
          gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, new Uint16Array(coords[attribute]), gl.STATIC_DRAW);
          return {...model, [attribute]: {buffer, coords: [...coords[attribute]]}};
      }
      gl.bindBuffer(gl.ARRAY_BUFFER, buffer);
      gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(coords[attribute]), gl.STATIC_DRAW);
      return {...model, [attribute]: {buffer, coords: [...coords[attribute]]}};
  }, {});
};
